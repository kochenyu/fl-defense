...
def start_train():
    global poison_percent
    global all_poison_accurate
    global record_cos_value
    global record_cos_client_value
    all_poison_accurate = {}
    record_cos_value = {}
    record_cos_client_value = {}
    for poison_percent in range(5, 6):
        print("-----start train poison percent {}".format(poison_percent))
...
def model_training():
    ...
    for iter in range(args.epochs):
        ...
        if iter == 0:
            ...
            agg_midden_value = test.getAverageAggWellGlobalModel(midden_client_agg)
            median_grads = agg_midden_value[key_name]
            # median_client_grads = agg_midden_value['fc2.weight.grad']
            # 将之前的所有梯度分别于这个进行比较， 得到余弦相似度结果数组
            test.getL2TensorCosineSimilary(median_grads, data_grads_dict, result)
        else:
            # agg_midden_value = test.agg_midden_value(midden_client_agg)
            agg_midden_value = test.getAverageAggWellGlobalModel(midden_client_agg)
            # well = midden_client_agg[clients_in_comm[2]][key_name]
            # bad = midden_client_agg[clients_in_comm[0]][key_name]
            # j = test.getPearson(bad, well)
            # k = test.getSeverWeightValue(bad, well)
            compare_thred = test.getSeverWeightValue(his_well_weight_base[key_name], agg_midden_value[key_name])
            if compare_thred < client_cos_t:
                #         使用历史的进行判断恶意客户端
                median_grads = his_well_weight_base[key_name]
                test.getL2TensorCosineSimilary(median_grads, data_grads_dict, result)
                # 取良性客户端与历史全局 作为这轮的历史
                selected_midden_client_agg = test.filter_and_select(result, midden_client_agg)
                selected_midden_client_agg['his'] = his_well_weight_base
                his_well_weight_base = copy.deepcopy(test.getAverageAggWellGlobalModel(selected_midden_client_agg))
            else:
                median_grads = agg_midden_value[key_name]
                test.getL2TensorCosineSimilary(median_grads, data_grads_dict, result)
                well_code = test.check_threshold(result, client_cos_t)

        record_well_cos_client_value[iter] = copy.deepcopy(result)
        # 记录余弦相似度低于阈值的数据
        for key, value in result.items():
            if value < client_cos_t:
                # 该客户端属于中毒客户端
                record_client.append(key)

        # 获取全局均值模型
        w_glob = copy.deepcopy(test.getAverageAggWellGlobalModel(midden_client_agg))

        if iter == 0:
            save_well_weight_global["recover"] = copy.deepcopy(w_glob)
            # 发送聚合模型给聚集器
            his_well_weight_base = copy.deepcopy(w_glob)
            for key, value in w_glob.items():
                if key_name in key:  # 假设梯度是关于权重的
                    # if 'conv2.weight' in key:  # 假设梯度是关于权重的
                    compare_base = copy.deepcopy(torch.from_numpy(w_glob[key].cpu().numpy()))
                    # print(compare_base)
        else:
            for key, value in w_glob.items():
                # if 'conv2.weight' in key:  # 假设梯度是关于权重的
                if key_name in key:  # 假设梯度是关于权重的
                    compare_title = copy.deepcopy(torch.from_numpy(w_glob[key].cpu().numpy()))

        if len(compare_base) > 0 and len(compare_title) > 0:
            print("执行中毒攻击服务器聚集梯度比较")
            t = test.getSeverWeightValue(compare_base, compare_title)
            record_well_cos_agg_value[iter] = t
            #   根据t与阈值比较， 判断是否被污染 如果没被污染的话， 就迭代 title -> base
            if t > server_cos_t and well_code:
                # 将新的梯度覆盖以前的梯度
                save_well_weight_global["recover"] = copy.deepcopy(w_glob)
                his_well_weight_base = copy.deepcopy(w_glob)
                compare_base = copy.deepcopy(compare_title)
            else:
                # 取良性客户端与历史全局的平均 作为这轮的历史
                if len(selected_midden_client_agg) > 0:
                    w_glob = copy.deepcopy(his_well_weight_base)
                    save_well_weight_global["recover"] = copy.deepcopy(w_glob)
                else:
                    selected_midden_client_agg = test.filter_and_select(result, midden_client_agg)
                    selected_midden_client_agg['his'] = his_well_weight_base
        ...
        # 每次获取中值以后清空
        data_grads_dict.clear()
        midden_client_agg.clear()
        # 清除毒化客户端 重新生成
        poison_client.clear()
        result.clear()
        selected_midden_client_agg.clear()

        # w_glob = Fed.FedMidden(midden_client_agg)
        ...
        # loss_plot_list.append(test_loss)

        median_grads_dict.clear()
        accurate_list[iter] = acc_test.item()
    ...
    print("test accurate all record is:", accurate_list)
    all_poison_accurate[poison_percent] = accurate_list
    print("end")
